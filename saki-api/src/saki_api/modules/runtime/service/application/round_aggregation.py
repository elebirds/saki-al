...
def build_round_update_from_steps(*, round_row: Round, steps: Iterable[Step]) -> RoundUpdate:
    ...
    if round_row.mode == LoopMode.ACTIVE_LEARNING and snapshot.state == RoundStatus.COMPLETED:
        round_state = RoundStatus.WAIT_USER
    ...
    if (round_state in TERMINAL_ROUND_STATES or round_state == RoundStatus.WAIT_USER) and not round_row.ended_at:
