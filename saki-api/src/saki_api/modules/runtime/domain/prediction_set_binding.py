"""Frozen class mapping snapshot for prediction_set."""
...
class PredictionSetBinding(UUIDMixin, TimestampMixin, SQLModel, table=True):
    __tablename__ = "prediction_set_binding"

    prediction_set_id: uuid.UUID = Field(foreign_key="prediction_set.id", index=True, unique=True)
