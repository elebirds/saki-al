...
import { Card, Descriptions, Tag, Spin, message, Space, Divider, Typography, List, Avatar, Collapse } from 'antd';
  ...
  return (
    <div className="flex h-full flex-col gap-6 overflow-hidden p-6">
      ...
      <div className="flex-1 overflow-y-auto pr-2">
        <div className="rounded-2xl border border-github-border bg-gradient-to-r from-[var(--github-panel)] via-[var(--github-base)] to-[var(--github-panel)] p-6">
          <div className="flex flex-wrap items-center gap-4">
            <Avatar size={64} icon={<UserOutlined />} className="bg-gradient-to-br from-orange-400 to-pink-500">
              {displayInitial}
            </Avatar>
            <div>
          ...
          <div className="mt-4 flex flex-wrap gap-6 text-xs text-github-muted">
            ...
            {displayUser.updatedAt && <span>{t('userProfile.updatedAt')}: {formatDate(displayUser.updatedAt)}</span>}
            {displayUser.lastLoginAt && <span>{t('userProfile.lastLoginAt')}: {formatDate(displayUser.lastLoginAt)}</span>}
          </div>
        </div>
