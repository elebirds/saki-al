...
class PermissionChecker:
    ...
    async def get_user_system_permissions(self, user_id: uuid.UUID) -> Set[str]:
        ...
        """
        return await self.permission_repo.get_user_system_permissions(user_id, datetime.now(UTC))
    ...
    async def is_super_admin(self, user_id: uuid.UUID) -> bool:
        ...
        """
        return await self.user_role_repo.has_super_admin_role(user_id, datetime.now(UTC))
    ...
    async def is_admin(self, user_id: uuid.UUID) -> bool:
        ...
        avoiding the need to fetch all roles and check individually.
        """
        return await self.user_role_repo.has_admin_role(user_id, datetime.now(UTC))
    ...
    async def check(
        ...
        Examples:
            ...
            from saki_api.modules.access.domain.rbac import parse_permission
    ...
    async def filter_accessible_resources(
        ...
        Example:
            from saki_api.modules.access.domain.rbac import ResourceType, parse_permission
