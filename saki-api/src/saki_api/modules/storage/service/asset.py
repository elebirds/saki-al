...
from saki_api.infra.db.query import Pagination, OrderByType
from saki_api.infra.db.transaction import transactional
from saki_api.infra.storage.provider import get_storage_provider, StorageError
...
from saki_api.modules.storage.api.asset import AssetRead, AssetCreate, AssetUpdate
from saki_api.modules.storage.domain.asset import Asset
from saki_api.modules.storage.repo.asset import AssetRepository


class AssetService(CrudServiceBase[Asset, AssetRepository, AssetCreate, AssetUpdate]):
    """
    ...
    async def list_orphaned_assets_older_than(
            self,
            *,
            older_than: datetime,
            limit: int = 1000,
    ) -> List[Asset]:
        """List orphaned asset candidates for GC."""
        return await self.repository.get_orphaned_assets_older_than(
            older_than=older_than,
            limit=limit,
        )

    async def delete_asset_record(self, asset_id: uuid.UUID) -> bool:
        """Delete only DB record; caller controls transaction boundary."""
