...
type SystemInitializationState string

const (
	SystemInitializationStateUninitialized SystemInitializationState = "uninitialized"
	SystemInitializationStateInitialized   SystemInitializationState = "initialized"
)

func (e *SystemInitializationState) Scan(src interface{}) error {
	switch s := src.(type) {
	case []byte:
		*e = SystemInitializationState(s)
	case string:
		*e = SystemInitializationState(s)
	default:
		return fmt.Errorf("unsupported scan type for SystemInitializationState: %T", src)
	}
	return nil
}

type NullSystemInitializationState struct {
	SystemInitializationState SystemInitializationState `json:"system_initialization_state"`
	Valid                     bool                      `json:"valid"` // Valid is true if SystemInitializationState is not NULL
}

// Scan implements the Scanner interface.
func (ns *NullSystemInitializationState) Scan(value interface{}) error {
	if value == nil {
		ns.SystemInitializationState, ns.Valid = "", false
		return nil
	}
	ns.Valid = true
	return ns.SystemInitializationState.Scan(value)
}

// Value implements the driver Valuer interface.
func (ns NullSystemInitializationState) Value() (driver.Value, error) {
	if !ns.Valid {
		return nil, nil
	}
	return string(ns.SystemInitializationState), nil
}
