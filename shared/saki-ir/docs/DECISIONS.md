...
| OBB 使用 center + size + `angle_deg_ccw`（屏幕坐标 CCW） | OBB 旋转表达更稳定，便于几何推导和顶点计算；与 `θ=+90` 指向 `+y` 的屏幕直觉一致 | 必须区分 local/screen 顶点顺序；实现和测试需锁定 `angle_deg_ccw` 语义 |
...
| checksum 使用 CRC32C(Castagnoli) 且覆盖 encoded payload bytes | 支持先校验再解压/解码，降低坏包触发解压开销与风险 | `schema_version=2` 固化该语义；如需 raw 校验应新增独立字段 |
