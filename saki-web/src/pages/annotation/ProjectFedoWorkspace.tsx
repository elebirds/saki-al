...
const ProjectFedoWorkspace: React.FC<ProjectFedoWorkspaceProps> = ({ dataset }) => {
  ...
  const buildDraftPayload = useCallback((dualAnnotations: Annotation[]): AnnotationDraftPayload => {
    const items: AnnotationDraftItem[] = dualAnnotations.map((ann) => ({
  ...
  const buildPayloadFromState = useCallback((_: Annotation[]) => {
    ...
    return buildDraftPayload(baseAnnotations);
  }, [annotationState.annotations, annotationViews, generatedAnnotations, buildDraftPayload]);
  ...
  const { syncDraft } = useWorkingDraftPipeline({
    ...
    buildPayload: buildPayloadFromState,
  ...
  }, [applyBaseAnnotations, labelMap, projectId, currentSample?.id]);
