...
from saki_api.modules.access.domain.rbac import ResourceType, Role, RoleType
from saki_api.modules.access.domain.rbac.resource_member import ResourceMember
from saki_api.modules.access.service.presets import DATASET_OWNER_ROLE_ID, DATASET_ROLE_NAME_PREFIX
...
from saki_api.modules.project.service.field_overrides import get_or_override
from saki_api.modules.shared.application.crud_service import CrudServiceBase
from saki_api.modules.storage.api.dataset import DatasetCreate, DatasetUpdate
...
class DatasetService(CrudServiceBase[Dataset, DatasetRepository, DatasetCreate, DatasetUpdate]):
    """
    ...
    async def _is_supremo_role(self, role_id: uuid.UUID) -> bool:
        role = await self.access_gateway.get_role(role_id)
        if role is None:
    ...
    async def create_dataset(self, schema: DatasetCreate, owner_id: uuid.UUID) -> Dataset:
        ...
        await self.access_gateway.assign_resource_role(
    ...
    async def get_dataset_members(self, dataset_id: uuid.UUID) -> List[ResourceMemberRead]:
        ...
        result = await self.access_gateway.list_resource_members(
            resource_type=ResourceType.DATASET,
    ...
    async def add_dataset_member(
        ...
        existing = await self.access_gateway.get_member_by_user_and_resource(
        ...
        created = await self.access_gateway.create_member(new_member.model_dump())
        return created
    ...
    async def update_dataset_member(
        ...
        # Update member
        updated = await self.access_gateway.update_member(
            existing.id,
            {"role_id": member_data.role_id}
        )
    ...
    async def remove_dataset_member(
        ...
        # Delete member
        await self.access_gateway.delete_member(existing.id)
