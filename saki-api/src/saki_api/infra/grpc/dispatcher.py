...
class RuntimeDispatcher:
    ...
    async def _load_task_payload(self, task_id: uuid.UUID) -> TaskDispatchPayloadDTO | None:
        async with SessionLocal() as session_db:
            task = await session_db.get(JobTask, task_id)
            if not task:
                return None
            ...
            return TaskDispatchPayloadDTO(
                ...
                plugin_id=str(job.plugin_id or ""),
                mode=job.mode.value,
                query_strategy=str(job.query_strategy or ""),
