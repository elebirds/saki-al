...
from saki_ir.quad8 import quad8_to_obb_payload
...
def _parse_poly8_to_obb(
    *,
    ...
    if ctx.yolo_is_normalized:
        nums = [
            float(nums[i]) * (float(image_w or 0) if i % 2 == 0 else float(image_h or 0))
            for i in range(8)
        ]
    ...
    try:
        payload = quad8_to_obb_payload(nums, fit_mode="strict", eps=ctx.eps)
    except Exception:
        try:
            payload = quad8_to_obb_payload(nums, fit_mode="min_area", eps=ctx.eps)
        except Exception:
            fail_or_report(
                ctx=ctx,
                report=report,
    ...
    obb_payload = payload.get("obb") if isinstance(payload, dict) else None
    if not isinstance(obb_payload, dict):
        fail_or_report(
            ctx=ctx,
            report=report,
            code=ERR_CONVERT_GEOMETRY,
            message="poly8 拟合 OBB 失败",
            source_ref=source_ref,
        )
        return None

    return annotationirv1.ObbGeometry(
        cx=float(obb_payload.get("cx", 0.0)),
        cy=float(obb_payload.get("cy", 0.0)),
        width=float(obb_payload.get("width", 0.0)),
        height=float(obb_payload.get("height", 0.0)),
        angle_deg_ccw=float(obb_payload.get("angle_deg_ccw", 0.0)),
    )
